...
pub fn render(frame: &mut Frame, app: &App) {
    ...
    match app.mode {
        ...
        ViewMode::ConfirmPopup => {
            ...
            overlay::render_confirm(frame, frame.area(), &app.confirm_item);
        }
        ViewMode::FormPopup => {
            render_dashboard(frame, vert[1], app, app.active_panel, &app.global_filter);
    ...
    let hints = match app.mode {
        ViewMode::Dashboard => panel::get_dashboard_hints(app.active_panel),
    ...
    if let Some((title, msg)) = &app.result { overlay::render_result(frame, frame.area(), title, msg); }
}
...
fn render_dashboard(frame: &mut Frame, area: Rect, app: &App, active: usize, filter: &str) {
    ...
    let s3c = panel::panel_cols(top[0].width, &[("Name", 6.0, left), ("Created", 4.0, left)]);
    panel::render_panel(frame, top[0], app, 0, filter, " S3 BUCKETS ", &s3c,
        &app.s3_buckets, |b: &crate::types::S3Bucket| vec![b.name.clone(), b.created.clone()], &[]);
    let emrc = panel::panel_cols(top[1].width, &[("Name", 6.0, left), ("State", 4.0, left)]);
    panel::render_panel(frame, top[1], app, 1, filter, " EMR VIRTUAL CLUSTERS ", &emrc,
    ...
    let cogc = panel::panel_cols(mid[0].width, &[("Name", 4.0, left), ("Status", 3.0, left), ("Email", 3.0, left)]);
    panel::render_panel(frame, mid[0], app, 2, filter, " COGNITO USERS ", &cogc,
        &app.cognito_users, |u: &crate::types::CognitoUser| vec![u.username.clone(), u.status.clone(), u.email.clone()], &[]);
    let secc = panel::panel_cols(mid[1].width, &[("Name", 5.0, left), ("Rot", 2.0, left), ("Status", 3.0, left)]);
    panel::render_panel(frame, mid[1], app, 3, filter, " SECRETS MANAGER ", &secc,
    ...
    let lamc = panel::panel_cols(bottom[0].width, &[("Name", 5.0, left), ("Runtime", 3.0, left), ("Timeout", 2.0, right)]);
    panel::render_panel(frame, bottom[0], app, 4, filter, " LAMBDA FUNCTIONS ", &lamc,
        &app.lambda_funcs, |f: &crate::types::LambdaFn| vec![f.name.clone(), f.runtime.clone(), format!("{}s", f.timeout)], &[]);
    let logc = panel::panel_cols(bottom[1].width, &[("Level", 2.0, left), ("Timestamp", 3.0, left), ("Message", 5.0, left)]);
    panel::render_panel(frame, bottom[1], app, 5, filter, " LOGS ", &logc,
