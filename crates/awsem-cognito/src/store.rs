...
pub fn list_users(conn: &DbConn, pool_id: &str) -> Result<Vec<CognitoUser>, String> {
    let c = conn.lock().map_err(|e| e.to_string())?;
    let mut stmt = c.prepare(
        "SELECT id, pool_id, username, password_hash, email, attributes_json, status FROM cognito_users WHERE pool_id = ?1",
    ).map_err(|e| e.to_string())?;
...
pub fn delete_user(conn: &DbConn, pool_id: &str, username: &str) -> Result<(), String> {
    let c = conn.lock().map_err(|e| e.to_string())?;
    c.execute(
        "DELETE FROM cognito_users WHERE pool_id = ?1 AND username = ?2",
