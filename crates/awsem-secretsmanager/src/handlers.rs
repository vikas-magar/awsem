...
fn find_secret(input: &Value, state: &AppState) -> Result<store::Secret, HttpResponse> {
    ...
    store::get_secret_by_name(&state.db, secret_id)
...
pub async fn handle(
    ...
    match target {
        ...
        "secretsmanager.DescribeSecret" => describe_secret(input, &state).await,
        "secretsmanager.ListSecrets" => list_secrets(&state).await,
...
async fn create_secret(input: Value, state: &AppState) -> HttpResponse {
    ...
    if name.is_empty() {
        return awsem_core::error::AwsemError::InvalidRequest("Name is required".into()).secrets_response();
    }
...
async fn get_secret_value(input: Value, state: &AppState) -> HttpResponse {
    ...
    if secret.deleted_at.is_some() {
        return awsem_core::error::AwsemError::NotFound("Secret is deleted".into()).secrets_response();
    }
    let version = match store::get_latest_version(&state.db, &secret.id) {
        Ok(v) => v,
        Err(e) => return awsem_core::error::AwsemError::Internal(e).secrets_response(),
    };
...
async fn delete_secret(input: Value, state: &AppState) -> HttpResponse {
    ...
    HttpResponse::Ok().json(json!({"ARN": secret.arn, "Name": secret.name, "DeletionDate": chrono::Utc::now().to_rfc3339()}))
}
async fn describe_secret(input: Value, state: &AppState) -> HttpResponse {
    let secret = match find_secret(&input, state) { Ok(s) => s, Err(e) => return e };
    HttpResponse::Ok().json(json!({
        "ARN": secret.arn, "Name": secret.name,
        "Description": secret.description, "KmsKeyId": secret.kms_key_id,
        "CreatedDate": secret.created_at, "LastChangedDate": secret.last_changed,
        "Tags": serde_json::from_str::<Value>(&secret.tags_json).unwrap_or(json!([])),
    }))
}
async fn list_secrets(state: &AppState) -> HttpResponse {
    match store::list_secrets(&state.db) {
        Ok(secrets) => {
            let list: Vec<Value> = secrets.into_iter().map(|s| json!({
                "ARN": s.arn, "Name": s.name,
                "Description": s.description, "KmsKeyId": s.kms_key_id,
                "CreatedDate": s.created_at, "LastChangedDate": s.last_changed,
